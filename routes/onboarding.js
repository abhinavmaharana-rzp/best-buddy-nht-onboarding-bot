...
module.exports = (boltApp) => {
  ...
  router.post("/trigger", async (req, res) => {
    ...
    try {
      ...
      await sendOnboardingPlan(boltApp, userId, {
        userName,
        userFunction,
        subFunction,
      });
  ...
  const createWelcomeBlocks = (
    ...
    return [
      ...
      { type: "divider" },
      {
        ...
        text: {
          type: "mrkdwn",
          text: "📺 *Recommended Slack Channels to Join*\n\n*For New Joiners*\n• #newbies\n• #best-nht-channel\n• #learning-and-development-razorpay\n\n*Tech & IT*\n• #tech_it\n• #tech_backend\n• #tech_oncall\n\n*Office & Food*\n• #office\n• #fnb_crews\n• #food-and-beverage",
        },
      },
  ...
  const createOnboardingBlocks = (selectedOnboardingData) => {
    return [
      { type: "divider" },
      {
        ...
        text: {
          type: "mrkdwn",
          text: `✅ *Here's your 30 days Onboarding Plan:*`,
        },
      },
      {
        type: "section",
        ...
        accessory:
          status === "completed"
            ...
            : {
                ...
                text: {
                  type: "plain_text",
                  text: status === "pending" ? "⏳ Pending" : "Mark Complete",
                  emoji: true,
                },
                action_id:
                  status === "pending"
  ...
  async function sendOnboardingPlan(boltApp, userId, userData) {
    ...
    const selectedOnboardingData =
      subFunction === "SR" ? srOnboardingData : onboardingData;
    ...
    const welcomeBlocks = createWelcomeBlocks(
      userName,
      userFunction,
      subFunction,
      checklistItems,
  ...
  boltApp.action(/show_day_(\d+)_(\d+)/, async ({ ack, body, client }) => {
    await ack();
    const [weekIndexStr, dayIndexStr] = body.actions[0].action_id
      .split("_")
      .slice(2);
    ...
    const completedTaskIndices = new Set(
      completedTasks.map((task) => task.taskIndex),
    );

    // Create and send task blocks
    const taskBlocks = createTaskBlocks(
      day,
  ...
  boltApp.action(/mark_complete_\d+_\d+_\d+/, async ({ ack, body, client }) => {
    await ack();
    const [, , weekIndexStr, dayIndexStr, taskIndexStr] =
      body.actions[0].action_id.split("_");
    ...
    try {
      ...
      const statusBlocks = createTaskStatusBlocks(
        task,
        "pending",
        weekIndex,
        dayIndex,
  ...
  boltApp.action(/approve_task_(.+)/, async ({ ack, body, client }) => {
    ...
    try {
      ...
      // Get task details
      const approvedTask =
        onboardingData[approval.weekIndex].days[approval.dayIndex].events[
          approval.taskIndex
        ];
  ...
  router.get("/progress/:userId", authMiddleware, async (req, res) => {
    try {
      ...
      const overallProgress = {
        checklist: {
          percentage: checklistProgress.total
            ? (checklistProgress.completed / checklistProgress.total) * 100
            : 0,
          ...checklistProgress,
        },
        onboarding: {
          percentage: onboardingProgress.total
            ? (onboardingProgress.completed / onboardingProgress.total) * 100
            : 0,
          ...onboardingProgress,
        },
        total: {
          percentage:
            checklistProgress.total + onboardingProgress.total
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      try {
        ...
        res
          .status(500)
          .json({ error: "Error marking checklist item as complete" });
      }
    },
  );
  ...
  router.get("/tasks/:userId", authMiddleware, async (req, res) => {
    try {
      ...
      const tasks = await TaskStatus.find({ userId }).sort({
        weekIndex: 1,
        dayIndex: 1,
        taskIndex: 1,
      });
      ...
      const checklistItems = await ChecklistItem.find({ userId }).sort({
        weekIndex: 1,
        dayIndex: 1,
        itemIndex: 1,
      });
      ...
      const userFunction =
        userInfo.user.profile.fields?.Xf0DMHFDQA?.value || "Unknown";
      const subFunction =
        userInfo.user.profile.fields?.Xf0DMHFDQB?.value || "Unknown";
        ...
        weeks: onboardingPlan.weeks.map((week, weekIndex) => ({
          ...
          days: week.days.map((day, dayIndex) => ({
            ...
            checklistItems: day.checklistItems.map((item, itemIndex) => {
              const itemStatus = checklistItems.find(
                (i) =>
