...
function createRouter(app) {
  ...
  router.get(
    "/overview",
    authMiddleware,
    cacheMiddleware("overview"),
    async (req, res) => {
      try {
        debugLog("Fetching overview data");

        const [pendingApprovals, uniqueUsers, allTasks, allChecklistItems] =
          await Promise.all([
        ...
        const approvalDetails = await Promise.all(
          approvals.map(async (approval) => {
            try {
              ...
              debugLog(
                `Error fetching approval details for ${approval._id}:`,
                error,
              );
              return {
                ...approval,
  ...
  router.get("/user/:userId", authMiddleware, async (req, res) => {
    try {
      ...
      console.error(
        `Error fetching user details for ${req.params.userId}:`,
        error,
      );
  ...
  router.post("/trigger-onboarding", authMiddleware, async (req, res) => {
    try {
      ...
      await sendOnboardingPlan(app, userId, {
        userName,
        userFunction,
        subFunction,
      });
